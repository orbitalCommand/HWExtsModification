export async function getSkins() {
  const hs = await Send(
    ...
    .then((r) => Object.values(r.results[0].result.response))
    .then((hs) => hs.map(({ id, power, skins }) => ({ id, power, skins })));
  ...
  return hs.map((h) => {
    const skins = {
      ...Object.values(lib.data.skin)
        .filter((s) => s.heroId == h.id)
        .reduce((acc, v) => {
          acc[v.id] = 0;
          return acc;
        }, {}),
    ...
    let empties = [];
    const costs = Object.keys(skins)
      .map((s) =>
        Object.values(lib.data.skin[s].statData.levels).filter(
          (l) => l.level > skins[s]
        )
      )
      .flat()
  ...
  c.forEach((h) => {
    let v = Object.keys(h.cost).map((k) => {
      return Object.keys(h.cost[k]).map((c) => {
        return `${cheats.translate(`LIB_${k.toUpperCase()}_NAME_${c}`)}(${
          h.cost[k][c]
        })`;
      });
    });
    console.log(
      `${cheats.translate(`LIB_HERO_NAME_${h.id}`)}: ${v.join(" / ")}`
    );
  });
}
